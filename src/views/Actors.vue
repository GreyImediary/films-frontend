        ...
        <template v-slot:top>
          ...
          >
            ...
            <v-dialog v-model="dialogDelete" max-width="500px">
              <v-card>
                <v-card-title class="text-h5">Вы уверены, что хотите удалить этого актёра?</v-card-title>
                <v-card-actions>
                  <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...mapActions(['loadActors', 'createActor', 'updateActor', 'deleteActor']),

    async loadData (page = 1) {
      const body = await this.loadActors({ page: page });
    ...
    async save () {
      if (this.editedIndex > -1) {
        const body = await this.updateActor({ actorId: this.editedItem.id, params: this.editedItem });
        this.items.splice(this.editedIndex, 1);
        this.items[this.editedIndex] = body.data;
      } else {
        await this.createActor(this.editedItem);
        await this.loadData();
      }
      this.close()
    },
